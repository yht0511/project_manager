...
gpt_model = "gpt-4"
gpt_prompt = """
*** 以下是一个项目的readme文件内容,我需要你根据文本信息,给出你判断出的这个项目的开发进度(百分比,尽可能精确)以及项目状态(分为“构思”、“开发”、“维护”、“归档”、“中止”)，构思是指项目还没有正式开始建设，只有基本的雏形和方案，没有一个阶段目标被完成；开发是指正在编写代码、设计模型或进行实验；维护是指开发已完毕，项目已经运行，平时对项目进行小修小补；归档是指项目已经不再维护；中止是指在开发或构思阶段决定放弃该项目；而开发进度是指”开发的进度“，若为构思，则进度为0；若为维护或归档，则进度为100；若为中止或开发，需要你根据readme中的各种信息来判断项目进行到何种程度.不要输出任何多余的话，直接使用json格式输出，格式如下: {"status": 项目状态,"progress": 开发进度} ***
"""
